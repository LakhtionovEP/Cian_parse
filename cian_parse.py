...
def get_cian_data(url):
    ...
    metro_str = ''
    a = []
    metro_time = filrd.findall(str(obj))
    if len(metro_time) == 0:
        metro_time = 'N/A'
    try:
        metro_name = filr.findall(str(obj2))
    except:
        metro_name = 'N/A'
    try:
        flat_price = filr.findall(str(obj3))[-1]
    except:
        flat_price = 'N/A'
    try:
        phone = filr.findall(str(obj4))
        phone_str = ',\n'.join(phone)
    except:
        phone_str = 'N/A'
    try:
        flat_price_pm = filr.findall(str(obj5))[-1]
    except:
        flat_price_pm = 'N/A'
    try:
        address = filra.findall(str(obj6))
        ...
        add_str = 'N/A'
    try:
        square = filr.findall(str(obj7))[-1]
    except:
        square = 'N/A'
    try:
        room_q = filr.findall(str(obj8))[-1].split(',')[0]
    except:
        room_q = 'N/A'
    try:
        cur_fl, ov_fl = filrfl.findall(str(obj9))[-1].split(' из ')
    except:
        cur_fl, ov_fl = 'N/A', 'N/A'
    try:
        b_type = fillrtype.search(str(obj10))[0]
    except:
        b_type = 'N/A'
    try:
        for i in range(len(metro_name)):
...
print('Какой формат аналогов? (4 или 5)')
while True:
    ana_qua = input()
    if ana_qua == '4':
